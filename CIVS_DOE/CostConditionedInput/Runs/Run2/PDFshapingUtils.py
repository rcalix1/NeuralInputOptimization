...
class PDFshapingUtils:
    ...
    def __init__(self):
        ...
        self.z                  = None
        self.batch_size_syn     = 32
        self.device             = 'cuda' if torch.cuda.is_available() else 'cpu'
        
        ## [1.0, 3.0, 10.0]
        self.constraint_weights = torch.tensor([1.0, 1.0, 1.0, 1.0], device=self.device).repeat(self.batch_size_syn, 1)
    ...
    def init_z_constraints(self):
        ...
        z_init = torch.rand((self.batch_size_syn, 7), device=self.device) * 0.4 + 0.3  # Middle of sigmoid range
    ...
    def get_clamps_min_max_constraints(self, batch_size, k_range=(1.5, 2.5), device=None):
        ...
        delta = 50.0  # or any scalar value
        clamp_min_single =  clamp_min_single - delta
        clamp_min_single = torch.clamp(  clamp_min_single, min=0.0  )
        
        clamp_max_single =  clamp_max_single + delta
    ...
    def get_upper_lower_bounds_constraints7(self):
       
        target_output_not_scaled = torch.tensor(
                    [[2600.0, 1800.0, 70.0]] * self.batch_size_syn,
                    device=self.device
        )
        
        margin = torch.tensor([100.0, 30.0, 10.0], device=self.device).unsqueeze(0).expand(self.batch_size_syn, -1)


        lower_bounds_glob = target_output_not_scaled.clone()
        upper_bounds_glob = target_output_not_scaled + margin
        
        y_means      = self.y_means.to(      self.device)
        y_deviations = self.y_deviations.to( self.device)

        lower_bounds_glob_scaled = (lower_bounds_glob - y_means) / y_deviations
        upper_bounds_glob_scaled = (upper_bounds_glob - y_means) / y_deviations


        return lower_bounds_glob, upper_bounds_glob, lower_bounds_glob_scaled, upper_bounds_glob_scaled
    
    
    
    def get_upper_lower_bounds_constraints88(self):
        mu  = self.y_means.to(self.device).reshape(-1)        # 
        std = self.y_deviations.to(self.device).reshape(-1)   # 

        # Correlation from dataset (fta, hmt, tgt) — numeric & no NaNs
        cols = ['o_fta', 'o_hmt', 'o_tgt', 'o_prod_rt']
        R_np = self.CFD_raw_data[cols].astype(float).dropna().corr().to_numpy()
        R    = torch.tensor(R_np, device=self.device, dtype=mu.dtype)

        # Σ = D R D  (+ jitter for Cholesky stability)
        D = torch.diag(std)
        Sigma = D @ R @ D
        ## Sigma = Sigma + 1e-6 * torch.eye(3, device=self.device, dtype=mu.dtype)
        Sigma = Sigma + 1e-6 * torch.eye(4, device=self.device, dtype=mu.dtype)

        # Correlated lower bounds
        L = torch.linalg.cholesky(Sigma)
        ## z = torch.randn(self.batch_size_syn, 3, device=self.device, dtype=mu.dtype)
        z = torch.randn(self.batch_size_syn, 4, device=self.device, dtype=mu.dtype)
        lower_bounds_glob = (mu + z @ L.T).clamp(mu - 3*std, mu + 3*std)

        # Random margins → upper
        k = torch.empty(self.batch_size_syn, 4, device=self.device, dtype=mu.dtype).uniform_(0.2, 1.2)
        upper_bounds_glob = lower_bounds_glob + k * std

        # Scaled
        lower_bounds_glob_scaled = (lower_bounds_glob - mu) / std
        upper_bounds_glob_scaled = (upper_bounds_glob - mu) / std
        return lower_bounds_glob, upper_bounds_glob, lower_bounds_glob_scaled, upper_bounds_glob_scaled


    def get_upper_lower_bounds_constraints(self):
        mu  = self.y_means.to(self.device).reshape(-1)        
        std = self.y_deviations.to(self.device).reshape(-1)   

        # Number of outputs (vector size) is now dynamic
        vec_size = mu.numel()

        # Correlation from dataset — numeric & no NaNs
        cols = ['o_fta', 'o_hmt', 'o_tgt', 'o_prod_rt']  # Keep your chosen order
        R_np = self.CFD_raw_data[cols].astype(float).dropna().corr().to_numpy()
        R    = torch.tensor(R_np, device=self.device, dtype=mu.dtype)
        ...
        D = torch.diag(std)
        Sigma = D @ R @ D
        Sigma = Sigma + 1e-6 * torch.eye(vec_size, device=self.device, dtype=mu.dtype)

        # Correlated lower bounds
        L = torch.linalg.cholesky(Sigma)
        z = torch.randn(self.batch_size_syn, vec_size, device=self.device, dtype=mu.dtype)
        lower_bounds_glob = (mu + z @ L.T).clamp(mu - 3*std, mu + 3*std)  # 3 = std scale

        # Random margins → upper
        k = torch.empty(self.batch_size_syn, vec_size, device=self.device, dtype=mu.dtype).uniform_(0.2, 1.2)
        upper_bounds_glob = lower_bounds_glob + k * std
    ...
    def sample_input_from_dist_fn(self):
        
        mu  = self.x_means.to(self.device).reshape(-1)        
        std = self.x_deviations.to(self.device).reshape(-1)   
        ...
        cols = ['i_h2i_rate', 'i_h2_temp', 'i_ngi_rate', 'i_pci_rate', 'i_o2_volfract', 'i_hbtemp', 'i_wind_rt']  
        ...
        clamp_min = torch.zeros_like(mu) 
        ## lower_bounds_glob = (mu + z @ L.T).clamp(mu - 3*std, mu + 3*std)  # 3 = std scale
        lower_bounds_glob    = (mu + z @ L.T).clamp(min=clamp_min, max=mu + 3*std)
      
        
        

        return lower_bounds_glob

    

    
    
    def load_synth_folder_to_pandas(self, folder="SyntheticCorpus", flatten=True, sort=True):
        """
        Reads all *.json sample files from `folder` into a pandas DataFrame.
        If `flatten=True`, list-valued columns are expanded into numeric columns.
        """
        files = glob.glob(os.path.join(folder, "*.json"))
        if sort:
            # deterministic order by filename (or use os.path.getmtime for mtime)
            files = sorted(files)
        ...
        # infer sizes from first row
        n_in  = len(df["final_input"].iloc[0])
        n_out = len(df["final_output"].iloc[0])

        def expand_list_col(df_in, col, n, prefix):
            return pd.DataFrame(df_in[col].to_list(), columns=[f"{prefix}_{j}" for j in range(n)])

        # expand input-sized arrays
        for col in ["clamp_min", "final_input", "clamp_max"]:
            df = df.join(expand_list_col(df, col, n_in, col)).drop(columns=[col])

        # expand output-sized arrays
        for col in ["lower_bounds_glob", "final_output", "upper_bounds_glob"]:
            df = df.join(expand_list_col(df, col, n_out, col)).drop(columns=[col])
    ...
    def r2_score_torch(self, y_true, y_pred):
        ...
        # Mean of true values along samples axis
        mean_true = torch.mean(y_true, dim=0, keepdim=True)

        ss_res = torch.sum((y_true - y_pred) ** 2, dim=0)
        ss_tot = torch.sum((y_true - mean_true) ** 2, dim=0)
