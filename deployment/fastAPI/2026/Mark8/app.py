...
######################################
import matplotlib
matplotlib.use("Agg")               ## Not sure if this is MAC specific 
...
h2_rate        = config.getfloat("VARIABLES", "h2_rate")
pci_rate       = config.getfloat("VARIABLES", "pci_rate")
h2_temp        = config.getfloat("VARIABLES", "h2_temp")
weight_MSE_ini = config.getfloat("VARIABLES", "weight_MSE_ini")

###########################################################
    
global_price_pci_rate_ini        = config.getfloat("VARIABLES", "global_price_pci_rate_ini")
global_price_ngi_rate_ini        = config.getfloat("VARIABLES", "global_price_ngi_rate_ini")      
global_price_coke_rate_ini       = config.getfloat("VARIABLES", "global_price_coke_rate_ini")   
global_price_wind_rate_ini       = config.getfloat("VARIABLES", "global_price_wind_rate_ini") 
global_price_o2_volfract_ini     = config.getfloat("VARIABLES", "global_price_o2_volfract_ini") 

###########################################################

print("Printing variables from .ini file")
print(h2_rate, pci_rate, h2_temp)


########################################################################################################

### global_price_pci_rate    = 0.3
### global_price_ngi_rate    = 0.055            ## was 0.2
### global_price_coke_rate   = 0.45       
### global_price_wind_rate   = 0.1
### global_price_o2_volfract = 1


global_price_pci_rate    = global_price_pci_rate_ini 
global_price_ngi_rate    = global_price_ngi_rate_ini             
global_price_coke_rate   = global_price_coke_rate_ini       
global_price_wind_rate   = global_price_wind_rate_ini
global_price_o2_volfract = global_price_o2_volfract_ini
...
def cost_func_h2i_rate_old(H2_rate=10, PCI_rate=25, H2_temp=600):
...
def cost_func_h2i_rate(H2_rate=10, PCI_rate=25, H2_temp=600):
    ...
    ## Cost per kg for a given temperature increase

    H6         = E6 * (E26-300)

    ## Cost per kg for a kg of H2

    E4         = 2

    ## Cost per kg for a kg of H2 raised to a given temperature increase

    E5         = E4 + H6


    ## Total cost of H2 for a given kg of H2 rate
...
def cost_func_o2_volfract( wind_rate=195, o2=0.215, prod_rate=5775):
    ...
    E9           = global_price_o2_volfract         #### 0.01 * 1000      ## HTML
...
def cost_func_ngi_rate( NG_rate=70 ):
    ...
    E8     = global_price_ngi_rate    ## 0.055     ## 0.20   HTML
...
def cost_func_coke_rate( coke_rate=470 ):
    ...
    E12         = global_price_coke_rate   ## 0.45
...
price_real    = np.array([0.0, global_price_pci_rate, global_price_ngi_rate, 0.0, 0.0, 0.0, 0.0])
price_real    = torch.tensor(price_real, dtype=torch.float32)

price_real_y5 = np.array([0.0, 0.0, 0.0, 0.0, global_price_coke_rate])
...
get_x_from_z_2 = lambda z: clamp_min + (clamp_max - clamp_min) * torch.sigmoid(z)


########################################################################################################


def get_x_from_z(z, clamp_min, clamp_max):
...
def encode_inputs_NIO(  request  ):
    ...
    return target_y, x_real, clamp_min, clamp_max, y_clamp_min,  y_clamp_max, y_clamp_min_temp, quad_exp_weight, sent_iters 
...
def generate_test_plot_by_name(name, y_data):
...
def generate_test_plot():
    ...
    # Random data
    x = np.arange(3000)
    y = np.cumsum(np.random.randn(3000))

    # Plot
    plt.figure(figsize=(8,4))
    plt.plot(x, y, linewidth=2)
    plt.title("Random Test Plot")
    plt.xlabel("Iteration")
    plt.ylabel("Value")
    plt.grid(True)

    # Save
    filename = "plots/test_plot.png"
    plt.savefig(filename, dpi=150, bbox_inches="tight")
    plt.close()
...
def NIO_main_optimization( request: OptimizeRequestNIO  ):
    ...
    target_y, x_real,  clamp_min, clamp_max, y_clamp_min, y_clamp_max, y_clamp_min_temp, quad_exp_weight, sent_iters     =    encode_inputs_NIO(  request  )
    ...
    for j in range( sent_iters ):              ## 3000
        
        optimizer.zero_grad()
        
        ##########################################

        ## only change was to replace lambda with this new function
        ################  x_temp_descaled              = get_x_from_z(    z    )   ## old lambda

        x_temp_descaled              = get_x_from_z(  z, clamp_min, clamp_max  )
        ...
        coke_history_plot.append(   y_descaled[:, 4].detach().item()  )
        fta_history_plot.append(    y_descaled[:, 3].detach().item()  )
        prod_history_plot.append(   y_descaled[:, 2].detach().item()  )
        hmt_history_plot.append(    y_descaled[:, 1].detach().item()  )
        tgt_history_plot.append(    y_descaled[:, 0].detach().item()  )
        ...
        cost_history_plot.append( loss_cost_descaled.detach() )
    ...
    generate_test_plot_by_name(   "tgt", tgt_history_plot )
    generate_test_plot_by_name(   "hmt", hmt_history_plot )
    generate_test_plot_by_name(  "prod", prod_history_plot )
    generate_test_plot_by_name(   "fta", fta_history_plot )
    generate_test_plot_by_name(  "coke", coke_history_plot )


    generate_test_plot_by_name(  "cost", cost_history_plot )
    ...
    current_request = request.model_dump()
    current_results = full_log_rc
...
def save_case():
    ...
    if current_request is None or current_results is None:
        return {"error": "No case has been run yet."}

    timestamp = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")

    filename = f"saved/{timestamp}.txt"
    ...
    with open(filename, "w") as f:
        ...
        f.write(json.dumps(current_request, indent=4))
    ...
    # Copy plots
    for plot in ["tgt", "hmt", "prod", "fta", "coke", "cost"]:

        shutil.copy(
            f"plots/{plot}.png",
            f"saved/{timestamp}_{plot}.png"
        )
